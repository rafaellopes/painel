...
- `countdown` — **deadline pressure on a human step**. A labelled deadline with a live countdown (days/h/m) that turns red and reads "overdue" once the time passes. `{ "type":"countdown", "label":"Submit the report by", "deadline":"2026-07-15T17:00:00", "done":false }`. `deadline` is an ISO-8601 local datetime; the countdown updates client-side (the server bakes in no clock). The human clicks **Done** → non-silent `countdown_done {block}` event (set `done:true`). Pending in the attention bar (labelled with `label`) until `done`. Reach for it when a human step has a real due date and the passing of time matters — not for open-ended asks.
- `rating` — **quick calibrated feedback** on a 1..`scale` star scale (`scale` capped at 10). `{ "type":"rating", "prompt":"How well did the email tone land?", "scale":5, "value":null, "labels":["poor","excellent"] }`. Optional `labels` are the low/high endpoints. The human clicks a star → non-silent `rate {block,value}` event (set `value`). Pending until `value` is set. Reach for it when you want a fast, comparable score (a 1–5 gut read), not free text — if you need the *reasons*, pair it with a `question` or use `form`.
- `table` — **structured data the human corrects and confirms**. `{ "type":"table", "title":"Suspicious transactions", "columns":[{"id":"date","label":"Date"},{"id":"desc","label":"Description"},{"id":"ok","label":"Confirm?","kind":"checkbox"}], "rows":[{"date":"2026-06-03","desc":"FX fee","ok":false}], "editable":["ok"], "confirmed":false }`. Read-only cells show as text; columns listed in `editable` render inputs (`kind:"checkbox"` → a checkbox, otherwise a text input). The grid scrolls horizontally inside its card. The human clicks **Confirm table** → non-silent `table_confirm {block,rows}` with the **full** current rows back (set `rows` to the returned value and `confirmed:true`). Pending until `confirmed`. Reach for it when you have several rows the human should eyeball and adjust in one pass (reconcile, tick off, fix a value) — not for a single field (use `form`/`question`).
- `gauge` — **one number that matters**, read-only. A bar + big number that turns warning-colored once it crosses a threshold. `{ "type":"gauge", "label":"Budget used", "value":7350, "max":10000, "unit":"€", "warn_at":0.8 }`. When `value ≥ warn_at·max` the number and bar switch to the warning color. No events, never pending — it's a status indicator, not an ask. Reach for it to make a single meaningful quantity (budget used, quota, progress toward a cap) glanceable; if you want the human to *act* on the number, pair it with a `question`/`approval`.
