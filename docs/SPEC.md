...
| **M2** | Batch-1 blocks (§5.2): the four still-unbuilt — `countdown`, `rating`, `table`, `gauge` (`file_drop`→`upload`/M15 and `links`→`resources`/M11 are already shipped and superseded) | Each passes §5.4 DoD (all 8 steps incl. `"<script>"`-in-every-field escaping); demo board shows all four; skill updated; the three interactive ones (`countdown`/`rating`/`table`) each render a static state under `ctx["export"]` (M3 §24.2) so export stays JS-free; `countdown` server-render is deterministic (JS computes remaining time client-side, no server clock in the golden) |
